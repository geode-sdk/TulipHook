...
std::vector<uint8_t> X64Generator::handlerBytes(
    ...
    auto runtimeInfo = this->runtimeInfoBytes(handler, a.buffer().size(), a.getLabel("handler-push"),
        a.getLabel("handler-alloc-mid"));
...
std::vector<uint8_t> X64Generator::wrapperBytes(int64_t original, int64_t wrapper, WrapperMetadata const& metadata) {
    ...
    auto runtimeInfo = this->runtimeInfoBytes(wrapper, a.buffer().size(), a.getLabel("wrapper-push"),
        a.getLabel("wrapper-alloc-mid"));
...
geode::Result<> X64Generator::relocateRIPInstruction(
    cs_insn* insn, uint8_t* buffer, uint64_t& trampolineAddress,
    uint64_t& originalAddress, int64_t disp
) {
...
geode::Result<> X64Generator::relocateBranchInstruction(
    cs_insn* insn, uint8_t* buffer, uint64_t& trampolineAddress,
    uint64_t& originalAddress, int64_t targetAddress, int64_t relocated, size_t originalTarget
) {
    auto const id = insn->id;
    auto const detail = insn->detail;
    auto const address = insn->address;
    ...
    if (difference <= 0x7fffffffll && difference >= -0x80000000ll) {
        return X86Generator::relocateBranchInstruction(insn, buffer, trampolineAddress, originalAddress, targetAddress,
            relocated, originalTarget);
    }
    ...
    if (id == X86_INS_JMP) {
        ...
        if (size == 2) {
            ...
            *relativeByte = -1;
            ...
            // what
            std::cerr << "Unhandled short branch relocation of " << insn->mnemonic << " with size=" << size <<
            std::endl;
        }
