...
public:
    ...
    static PushParameters fromOptcall(X86Assembler& a, AbstractFunction const& function) {
        ...
        for (auto& [oindex, param] : reordered) {
            ...
            // second primitive through edx
                ...
                registersUsed == 1 && param.m_kind == AbstractTypeKind::Primitive &&
                param.m_size <= sizeof(void*)
            ) {
    ...
    void generateDefaultCleanup() {
        ...
        if (
            std::holds_alternative<Register>(m_returnValueLocation) &&
            std::get<Register>(m_returnValueLocation) == Register::XMM0
        ) {
