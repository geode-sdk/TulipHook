...
namespace zz {
	namespace arm {
		...
		public:
			ThumbPseudoLabel(addr_t addr) :
				AssemblerPseudoLabel(addr) {}
		...
		public:
			ThumbRelocLabelEntry(bool is_pc_register) :
				RelocLabel(),
				ThumbPseudoLabel(0),
				is_pc_register_(is_pc_register) {}

			template <typename T>
			static ThumbRelocLabelEntry* withData(T value, bool is_pc_register) {
		...
		public:
			ThumbAssembler(void* address) :
				Assembler(address) {
				this->SetExecuteState(ThumbExecuteState);
			}

			ThumbAssembler(void* address, CodeBuffer* buffer) :
				Assembler(address, buffer) {
				this->SetExecuteState(ThumbExecuteState);
			}
			...
			// =====
			void t2_ldr(Register dst, MemOperand const& src) {
    ...
    if (rt.Is(pc)) {
      ...
      if (curr_pc % 4) {
		...
		public:
			ThumbTurboAssembler(void* address) :
				ThumbAssembler(address) {}

			ThumbTurboAssembler(void* address, CodeBuffer* buffer) :
				ThumbAssembler(address, buffer) {}
