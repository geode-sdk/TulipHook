...
typedef struct {
	...
	tinystl::unordered_map<intptr_t, intptr_t> relocated_offset_map;
...
#define _ turbo_assembler_->
	...
	if (op0 == 0b010001) {
		...
		if (op0 != 0b11) {
			...
			if (rs == 15) {
				DEBUG_LOG(
					"%d:relo <thumb1: add/sub/cmp/mov of pc> at %p",
	...
	if ((insn1 & 0xf800) == 0xf000 && (insn2 & 0x8000) == 0x8000) {
		...
		if ((op3 & 0b101) == 0b001) {
			...
			int32_t imm = SignExtend(
				(S << 24) | (i1 << 23) | (i2 << 22) | (imm10 << 12) | (imm11 << 1), 1 + 1 + 1 + 10 + 11 + 1, 32
			);
			addr_t dst_vmaddr = relo_cur_src_vmaddr(ctx) + imm;
			dst_vmaddr |= 1;
		...
		if ((op3 & 0b101) == 0b100) {
			...
			int32_t imm = SignExtend(
				(S << 24) | (i1 << 23) | (i2 << 22) | (imm10h << 12) | (imm10l << 2), 1 + 1 + 1 + 10 + 10 + 1, 32
			);
			addr_t dst_vmaddr = relo_cur_src_vmaddr(ctx);
			dst_vmaddr = ALIGN_FLOOR(dst_vmaddr, 4);
			dst_vmaddr += imm;
