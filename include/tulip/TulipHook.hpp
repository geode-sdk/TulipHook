...
namespace tulip::hook {
    ...
    TULIP_HOOK_DLL HookHandle createHook(
        HandlerHandle const& handler, void* address,
        HookMetadata const& metadata
    ) noexcept;
    ...
    TULIP_HOOK_DLL geode::Result<void, std::string> writeMemory(
    ...
    TULIP_HOOK_DLL geode::Result<void*, std::string> createWrapper(
        void* address,
        WrapperMetadata const& metadata
    ) noexcept;

    // wraps a function in given convention into cdecl
    TULIP_HOOK_DLL geode::Result<void*, std::string> createReverseWrapper(
        void* address,
        WrapperMetadata const& metadata
    ) noexcept;
    ...
    TULIP_HOOK_DLL RelocaledBytesReturn getRelocatedBytes(
        int64_t original, int64_t relocated,
        std::vector<uint8_t> const& originalBuffer
    );
    TULIP_HOOK_DLL std::vector<uint8_t> getCommonHandlerBytes(int64_t handler, ptrdiff_t spaceOffset);
    TULIP_HOOK_DLL std::vector<uint8_t> getCommonIntervenerBytes(
