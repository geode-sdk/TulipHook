...
#define FUNCTION_PARAM_TYPES int, int, int, int, int, int, int, int, int, float, float, float, float, float, float,    \
...
void makeHandler(HandlerHandle& out) {
    ...
    handlerMetadata.m_abstract = AbstractFunction::from<int32_t(
        int, int, int, int, int, int, int, int, int, float, float, float, float, float, float, float, float, float,
        float
        )>();
...
HookHandle makePriorityHook(HandlerHandle const& handle) {
    ...
    return createHook(handle,
        reinterpret_cast<void*>(static_cast<FunctionPtrType>(&priorityHook<N, FUNCTION_PARAM_TYPES>)), metadata);
}
...
int checkParams(
    int a, int b, int c, int d, int e, int f, int g, float h, int i, int j, int k, int l, int m, int n,
